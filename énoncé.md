...
2. une page d'authentification pour les utilisateurs (login.html)
...
Une démonstration de l'application est disponible ici : [https://youtu.be/myXpqbOJ8iw](https://youtu.be/myXpqbOJ8iw)
...
- **Scripts :** Des scripts JavaScript gèrent les fonctionnalités de filtrage, de manipulation de données et les actions utilisateur (comme la connexion et la gestion des livraisons). Le client est donc un client lourd qui communique avec l'API pour récupérer et mettre à jour les données.
...
| - **Appels API bien structurés**                    | Utilisation efficace de l’API pour les requêtes, bonnes pratiques REST respectées.                  | 25         |
...
La qualité pondère la quantité. Un projet complet mais mal codé peut avoir une note inférieure à un projet partiel mais bien codé. Pour calculer la note finale, on applique la formule suivante :

```js
noteFinale = (noteQuantite * noteQualite) / 100;
```

Si vous codez 50% des fonctionnalités mais que la qualité de votre code est parfaite, vous obtiendrez 50% de la note maximale. Mais si vous codez 100% des fonctionnalités mais que la qualité de votre code est mauvaise, vous obtiendrez une note inférieure à 50%. Si vous codez 50% des fonctionnalités et que la qualité de votre code est de 50%, vous obtiendrez une note de 25%.
...
- **Pénalités** :
  - **Retard** : Si le code n'est pas rendu (pour quelque raison que ce soit) la veille de l'examen, vous ne pourrez pas passer l'examen. Aucune dérogation ne sera prise en considération, puisque vous avez tous les jours avant la veille de l'examen pour déposer au moins une version de votre code.
